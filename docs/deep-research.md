...
In a single-host multi-process environment (e.g., pi-spine workers on one machine), rate limiting logic cannot rely on per-process memory; it must use a shared store. However, executing multiple discrete read and write commands without transactional boundaries introduces race conditions under concurrent load. To ensure atomic operations, the Token Bucket logic must run inside a single database transaction. For pi-smart-router MVP, **SQLite** (`better-sqlite3` with `BEGIN IMMEDIATE`) provides this on a local file at `.pi-smart-router/state.db` without an external server. For distributed multi-host deployments, an optional **Redis** adapter (post-MVP) can replace the store using Lua `EVAL` scripts for the same atomic read-refill-deduct semantics.
