# Task: SP-285 — privacy-safe embedding export

**Created:** 2026-09-10
**Size:** M

## Review Level: 1

**Assessment:** Post-1.0 calibration follow-up; bounded file scope.
**Score:** 2/8 — Blast radius: 1, Pattern novelty: 1, Security: 0, Reversibility: 0

## Source

- GitHub: beettlle/pi-smart-router#170
- Release: v1.0.1
- Manifest: `spine-tasks/_authoring/release-v1.0.1/manifest.md`

## Mission

Add opt-in privacy-safe embedding export + prompt_length completeness so HyDRA ≥100 floor can ever be met. No raw prompts.

## Dependencies

- SP-280

## Context to Read First

- `spine-tasks/CONTEXT.md`
- `spine-tasks/_authoring/release-v1.0.1/manifest.md`
- `docs/migration-v1.md` (honest-untrained 1.0 posture)
- Issue beettlle/pi-smart-router#170

## Environment

- **Workspace:** pi-smart-router
- **Services required:** None

## File Scope

| Scope | Paths |
|-------|-------|
| Must change | export path under `src/` or `scripts/` + tests |
| May change | README export docs |
| Must NOT change | `config/hydra` ship flip without floor |

## Contract

| Field | Value |
|-------|-------|
| testCommand | `npm run typecheck` |
| fileScopeMustChange | (see Must change in File Scope) |
| completionCriteria | Mission outcomes met; STATUS honest |

## Steps

### Step 0: Preflight

- [ ] Read CONTEXT + linked issue + migration honesty section
- [ ] Confirm v1.0 neutralize still in place until SP-284 ships trained artifacts

### Step 1: Implementation

- [ ] Deliver mission outcomes within File Scope

### Step 2: Testing & Verification

- [ ] Run contract testCommand
- [ ] Update STATUS with evidence


## Do NOT

- Store raw prompt text
- Claim HyDRA trained without meeting ≥100 floor

## Completion Criteria

- [ ] Mission complete without inventing labels or flipping #96 defaults
