 ...
 */
export function weightedSelect(candidates) {
...
export class GatewayDispatch {
     ...
     */
    async dispatchWithRateLimit(request, rateLimitKey) {
        if (this.rateLimiter) {
     ...
     */
    selectFailover(decision, excludeModelIds = []) {
        const excluded = new Set(excludeModelIds);
        const sameTier = this.fleet.filter((m) => m.tier === decision.tier &&
        ...
        const anyHealthy = this.fleet.filter((m) => m.id !== decision.selected_model_id &&
            !excluded.has(m.id) &&
            m.healthy !== false &&
            this.circuitBreaker.canDispatch(m.id));
     ...
     */
    updateCacheMarker(sessionId, decision) {
...
//# sourceMappingURL=gateway-dispatch.js.map