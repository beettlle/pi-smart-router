/**
 * RouterPipeline integration monolith.
 *
 * SP-277 (wave 1, #155) extracted stage-focused suites into sibling modules:
 *   router-pipeline-safe-default.test.ts   — safe-default.ts
 *   router-pipeline-session-pin.test.ts    — session-pin-stage.ts
 *   router-pipeline-hydra-match.test.ts     — hydra-match-stage.ts
 *   router-pipeline-stage-helpers.test.ts   — stage-helpers.ts (pure helpers)
 * Shared fixtures live in router-pipeline-fixtures.ts.
 * SP-278 (wave 2) fragments the remaining suites and retires this monolith.
 */
...
describe('RouterPipeline', () => {
  ...
  describe('concurrent route() safety (SP-230, #141)', () => {
    function makeConcurrencySystemInfo(): SystemInfo {
      return {
        totalMemoryGb: 16,
        arch: 'arm64',
        platform: 'darwin',
    ...
    function makeGatedProbe(events?: string[]) {
      ...
      const systemInfoProvider = async (): Promise<SystemInfo> => {
        ...
        return makeConcurrencySystemInfo();
      };
