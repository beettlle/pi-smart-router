...
#   1 — missing tools / wrong cwd / hard gates failed
#
...
if [[ ! -f package.json ]]; then
  echo "error: must run from pi-smart-router repo (package.json missing)" >&2
  exit 1
fi
...
HUMAN CHECKLIST (do in pi; this script does not drive live sessions)
  ...
  [ ] Fleet: economical + frontier + non-Gemini fallback (cursor/auto or composer-latest)
...
echo "==> Running hard fixture gates: npm run release:functional-smoke"
set +e
npm run release:functional-smoke >"$HARD_LOG" 2>&1
HARD_EC=$?
set -e
...
echo "==> Running TwinRouterBench soft-feed: npm run routing:assert-release-gates:corpus-report"
echo "    (report-only; soft FAIL on over-routing is expected and does not fail this script)"
set +e
npm run routing:assert-release-gates:corpus-report >"$SOFT_LOG" 2>&1
SOFT_EC=$?
set -e
